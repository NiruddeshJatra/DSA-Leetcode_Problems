...
# Then, for each subsequent element, we check if it's different from the element at position k-2.
#
...
#
# For example, with nums = [1,1,1,2,2,3]:
# - k starts at 2 (we keep nums[0]=1 and nums[1]=1)
...
# The final array is [1,1,2,2,3,...] with length 5.

# ALGO:
# 1. If the array has less than or equal to 2 elements, return the length of the array (all elements are valid).
...
#    a. If the current element is different from the element at position k-2:
...
class Solution:
   def removeDuplicates(self, nums: List[int]) -> int:
       ...
       for i in range(2, len(nums)):
           ...
           if nums[i] != nums[validPosition-2]:
